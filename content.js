...
const mockSustainabilityScores = {
  "amazon.com": {
    overall: "yellow",
    score: 65,
    factors: {
  ...
  "ajio.com": {
    ...
    reasoning:
      "Fast fashion model with significant environmental concerns. Limited transparency in supply chain practices."
  },
  "nykaa.com": {
    overall: "yellow",
    score: 62,
    ...
    reasoning:
      "Growing focus on sustainable beauty products but packaging and ingredient sourcing need improvement."
  }
}
...
function createSustainabilityBadge() {
  try {
    ...
    if (!isProductPage(currentUrl)) {
      console.log(
        "Sustainability Extension: Not a product page, skipping badge injection"
      )
      return
    }
    ...
    // Set badge content
    const scoreText =
      score.overall.charAt(0).toUpperCase() + score.overall.slice(1)
    badge.innerHTML = `
    ...
    badge.addEventListener("click", () => {
      try {
        ...
        alert(
          `Sustainability Score: ${scoreText} (${score.score}/100)\n\n${score.reasoning}`
        )
      }
    })
...
function removeSustainabilityBadge() {
  try {
...
window.sustainabilityExtension = {
