// Popup script for the sustainability extension
...
function getSustainabilityScore(url: string): SustainabilityScore {
  ...
  const mockScores: Record<string, SustainabilityScore> = {
    'amazon.com': {
      overall: 'yellow',
      score: 65,
...
function getAlternatives(count: number = 2): AlternativeProduct[] {
  const shuffled = [...mockAlternatives].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
}

// Get random reward
function getRewardOffer(): RewardOffer {
  return mockRewards[Math.floor(Math.random() * mockRewards.length)];
}

// Update score display
function updateScoreDisplay(score: SustainabilityScore): void {
  ...
  if (scoreBadge && scoreText && scoreReasoning && envScore && socialScore && govScore) {
    scoreBadge.className = `score-badge ${score.overall}`;
    scoreText.textContent = `${score.overall.charAt(0).toUpperCase() + score.overall.slice(1)} (${score.score}/100)`;
...
function updateAlternativesDisplay(alternatives: AlternativeProduct[]): void {
  ...
  alternatives.forEach(alt => {
    ...
    altElement.innerHTML = `
      ...
      <div class="certifications">
        ${alt.certifications.map(cert => `<span class="certification">${cert}</span>`).join('')}
      </div>
    `;
...
function updateRewardsDisplay(reward: RewardOffer): void {
...
// Update impact stats
function updateImpactStats(alternatives: AlternativeProduct[]): void {
